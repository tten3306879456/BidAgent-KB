...
### 新增
- **专家行业识别与路由机制**：招标解析专家新增强制行业识别（`industry` 标签），8 个流程专家全部接入行业标签路由
  ...
  - `general`（兜底）→ 标书法规库 + 标书案例库 + 投标文件编辑模板库（3 座通用底座）
  - 多行业冲突按「核工业 > 软件 > 设备 > 通用」优先级裁决；置信度 < 0.7 输出 `general` 并提示人工确认
- **行业垂直四件套补齐**（标准 + 案例 + 模板 + 种子，6 库共 23 个种子文件）：
  - 软件开发库 3→5 文件：新增 `软件行业废标案例与风险模式库.md`、`软件行业投标文件模板集.md`
  - 核工业库 2→4 文件：新增 `核工业废标案例与风险模式库.md`、`核工业投标文件模板集.md`
  - 设备采购库 4 文件：标准 + 废标案例 + 模板 + 种子齐备（标杆库）
- **排版专家接入云端投标文件编辑模板库兜底**（Tier 1 纯本地 → Tier 2 云端为辅型）
- 废标模式编号体系升级为行业系列：DQ-（设备）、RQ-（软件）、NQ-（核工业）

### 变更
- 9 个专家 Prompt 全部绑定 6 座真实 KB ID，`<YOUR_IMA_KB_ID>` 占位符清零
- 检索指令统一为「行业库废标案例优先 → 标书案例库通用模式 → 标书法规库法规验证」；报价与商务、资质等效不再作为独立库引用（并入标书法规库/标书案例库专题）
- `kb_config.json`：`seed_files` 19→23，软件类目 3→5、核工业类目 2→4，描述更新为「行业垂直四件套」
- `README.md`：架构图 5库15文件 → 6库23文件，新增设备库行，脚注改为行业路由说明
- `docs/知识库与专家匹配矩阵.md`：新增第 8 节「行业路由机制」（完整路由表 + 优先级裁决规则），9 专家引用明细全部补齐行业库行，P0/P1 缺口标注已解决
- `docs/交互界面原型.html`：5 色卡片区补齐软件 5 文件/核工业 4 文件/新增设备采购库卡片（#0EA5E9），"共 23 个文件"、6 座 KB 配置表
- `docs/新用户部署指南.html`、`guides/知识库使用指南.md`、`kb_data/使用指南.md`、`guides/共享知识库贡献指南.md`、`docs/微信分享卡片.html`、`docs/设计资产.md`：全部同步为 6 库 23 文件
...
### 新增
- **第 6 座共享知识库：设备采购知识库**（KB ID `7493501189104652`）——机械设备/信息化设备/实验室仪器/安防监控/教学实训设备等设备采购类项目知识
- **4 个设备采购类种子文件**（约 39KB，已全部上传入库）：
  - `seeds/设备采购标书知识库_种子版.md` — 87号令核心条款、评标与废标规则、验收要点
  - `seeds/设备采购技术标准库.md` — 机械/电气/信息化/实验室/安防等 30+ 标准要点与响应建议
  - `seeds/设备采购投标文件模板集.md` — 设备投标函、开标一览表、分项报价表、技术偏离表、制造商授权书
  - `seeds/设备采购废标案例与风险模式库.md` — 5 个真实废标案例 + EQ-001~010 风险模式 + 自检清单

### 变更
- `kb_config.json`：`shared_kbs` 新增 `equipment_procurement`，`seed_files` 新增 4 个文件，`shared_content.categories` 新增设备采购类目，版本描述 5→6 座
- `README.md`：种子文件 15→19、知识库 5→6（目录树/建库指引表/共享库列表/FAQ）
- `CONTRIBUTING.md`：贡献分类与社区共享库章节更新为 6 座 KB 体系
- `share/index.html`：徽章/功能卡片/知识库架构板块更新为 6 座（新增设备采购）
- `guides/知识库使用指南.md`：共享库清单 15→19 个文件、6 库内容概览表
- `seeds/ima建库操作指引.md`：文件清单更新为 6 库 19 文件
- `docs/部署指南.md`、`docs/设计资产.md`：种子文件数与知识库架构同步更新
- `scripts/kb_console.py`：SHARED_KBS 新增设备采购库定义（图标 ⚙️、关键词、建议上传）
- `scripts/kb_ima.py`：文档注释与示例配置更新为 6 座
